...
exports.layTatCaQuyTrinh = async (req, res) => {
  try {
    const danhSachQuyTrinh = await QuyTrinh.find()
      ...
      .select('ten maBenhNhan tenBenhNhan ngayBatDau trangThai buocHienTai diagramId ngayCapNhat');
    res.json(danhSachQuyTrinh);
  } catch (err) {
    res.status(500).json({ message: 'Lỗi khi lấy danh sách quy trình', error: err.message });
  }
};
...
exports.layQuyTrinhTheoId = async (req, res) => {
  try {
    const quyTrinh = await QuyTrinh.findById(req.params.id)
      .populate('cacBuoc')
      .populate('diagramId', 'name xml')
      .select('ten maBenhNhan tenBenhNhan ngayBatDau trangThai buocHienTai diagramId cacBuoc ngayCapNhat');
    if (!quyTrinh) {
      return res.status(404).json({ message: 'Không tìm thấy quy trình' });
    }
    res.json(quyTrinh);
  } catch (err) {
    res.status(500).json({ message: 'Lỗi khi lấy chi tiết quy trình', error: err.message });
  }
};

exports.taoQuyTrinhMoi = async (req, res) => {
  try {
    const { ten, maBenhNhan, tenBenhNhan, diagramId } = req.body;

    if (!ten || !maBenhNhan || !tenBenhNhan || !diagramId) {
    ...
    res.status(500).json({ message: 'Lỗi khi tạo quy trình', error: err.message });
  }
};

exports.capNhatQuyTrinh = async (req, res) => {
  try {
    const { ten, maBenhNhan, tenBenhNhan, trangThai, buocHienTai, diagramId, cacBuoc: updatedSteps } = req.body;
    ...
    if (tenBenhNhan) quyTrinh.tenBenhNhan = tenBenhNhan;
    ...
    if (updatedSteps && Array.isArray(updatedSteps)) {
      ...
      const populatedSteps = await Buoc.find({ _id: { $in: quyTrinh.cacBuoc } });
      const viTriHoanThanh = populatedSteps.reduce((maxIndex, b, index) => {
        return b.trangThai === 'hoan_thanh' ? index : maxIndex;
      }, -1);
      console.log("Vị trí hoàn thành cuối cùng:", viTriHoanThanh, "Tổng bước:", populatedSteps.length);

      if (viTriHoanThanh === populatedSteps.length - 1) {
        quyTrinh.buocHienTai = null;
        quyTrinh.trangThai = 'hoan_thanh';
      } else if (viTriHoanThanh >= 0 && viTriHoanThanh < populatedSteps.length - 1) {
    ...
    const quyTrinhCapNhat = await QuyTrinh.findById(req.params.id)
      ...
      .populate('diagramId', 'name xml')
      .select('ten maBenhNhan tenBenhNhan ngayBatDau trangThai buocHienTai diagramId cacBuoc ngayCapNhat');
    res.json({ message: 'Cập nhật quy trình thành công', quyTrinh: quyTrinhCapNhat });
  } catch (err) {
    res.status(500).json({ message: 'Lỗi khi cập nhật quy trình', error: err.message });
  }
};
...
exports.xoaQuyTrinh = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Lỗi khi xóa quy trình', error: err.message });
  }
};

exports.layTatCaSoDo = async (req, res) => {
  try {
    const danhSachSoDo = await Diagram.find().select('name xml createdAt updatedAt');
    res.json(danhSachSoDo);
  } catch (err) {
    res.status(500).json({ message: 'Lỗi khi lấy danh sách sơ đồ', error: err.message });
  }
};